...
1. **Data Source & Generation:** Simulated commercial transactions are generated by **Apache Airflow** and ingested into **PostgreSQL**, acting as the operational system.
2. **Change Data Capture (CDC): Debezium** monitors PostgreSQL's write-ahead logs (WAL) to capture row-level changes. These changes are converted into events and streamed into Apache Kafka topics in real-time.
3. **Stream Processing & ML Inference: PyFlink** consumes event streams from Kafka. The engine performs real-time feature engineering and invokes an integrated **XGBoost** model to perform fraud inference on-the-fly.
4. **Data Sinking & Visualization:** High-accuracy predictions are persisted back to **PostgreSQL. Grafana** then visualizes these insights through interactive dashboards, providing real-time observability.
